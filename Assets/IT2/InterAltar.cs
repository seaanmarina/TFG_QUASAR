...
public class InterAltar : MonoBehaviourPunCallbacks
{
   ...
   public GameObject interaccion;
    ...
    {
        if (control_blanca.contadorAsin >= 2 && poderPonerseBlanco==1)
        //if (control_blanca.contadorAsin >= 2 && !yaheentrado)
        {
            ...
            foreach (Photon.Realtime.Player player in players)
            {
                ...
                if (playerObject != null)
                {
                    ...
                    InterAltar playerController = playerObject.GetComponent<InterAltar>();
                    Debug.Log("El jugador variable es " + playerController.poderPonerseBlanco);
                    if (playerController != null && playerController.poderPonerseBlanco == 1)
                    {
